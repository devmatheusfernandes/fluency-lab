  ...
  try {
    ...
    if (!allowedTypes.includes(file.type)) {
      return NextResponse.json({ 
        error: 'Tipo de arquivo inválido. Por favor, selecione uma imagem (JPEG, PNG, GIF, WEBP).' 
      }, { status: 400 });
    }
    ...
    if (!storageBucket) {
      console.error('Firebase storage bucket not configured in environment variables');
      return NextResponse.json({ error: 'Storage not configured properly' }, { status: 500 });
    }
    ...
    if (oldAvatarFileName) {
      try {
        const oldFileRef = bucket.file(`avatars/${user.id}/${oldAvatarFileName}`);
