...
import { ContractCancellationRequest } from "@/components/contract/contrato-types";
...
export async function POST(
  ...
  try {
    ...
    if (!isOwnContract && !isAdmin) {
      return NextResponse.json(
        { success: false, message: "Não autorizado para cancelar este contrato" },
        { status: 403 }
      );
    }
    ...
    if (isOwnContract && !isAdmin) {
      ...
      if (!canCancel) {
        return NextResponse.json(
          { 
            success: false, 
            message: "Cancelamento só é permitido até 30 dias antes do vencimento" 
          },
          { status: 400 }
        );
      }
