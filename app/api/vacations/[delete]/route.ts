...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!vacationId) {
      return NextResponse.json({ error: 'ID do período de férias é obrigatório.' }, { status: 400 });
    }

    // Verificar se o período de férias pertence ao professor
    const vacation = await vacationRepository.findAllByTeacherId(session.user.id);
    const vacationExists = vacation.some(v => v.id === vacationId);
    
    if (!vacationExists) {
      return NextResponse.json({ error: 'Período de férias não encontrado ou não pertence ao professor.' }, { status: 404 });
    }
    ...
    if (vacationData) {
      ...
      if (now > earliestCancelDate) {
        return NextResponse.json({ 
          error: 'Período de férias não pode ser cancelado com menos de 40 dias de antecedência.' 
        }, { status: 400 });
      }
    }
    ...
    return NextResponse.json({ message: 'Período de férias cancelado com sucesso. As aulas afetadas foram reagendadas.' });
  } catch (error: any) {
