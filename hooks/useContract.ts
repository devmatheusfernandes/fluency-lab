...
interface UseContractActions {
  signContract: (signatureData: SignatureFormData) => Promise<ContractOperationResponse>;
  validateContract: () => Promise<ContractOperationResponse>;
  refreshContract: () => Promise<void>;
  dismissNotification: () => void;
}

export interface UseContractReturn extends UseContractState, UseContractActions {}
...
export const useContract = (): UseContractReturn => {
   ...
   */
  const signContract = useCallback(async (signatureData: SignatureFormData): Promise<ContractOperationResponse> => {
    if (!session?.user?.id) {
      return {
        success: false,
   ...
   */
  const validateContract = useCallback(async (): Promise<ContractOperationResponse> => {
    if (!session?.user?.id) {
      return {
        success: false,
        message: 'User not authenticated'
    ...
    try {
      const response = await fetch(`/api/contract/validate/${session.user.id}`, {
        method: 'POST',
      });
...
export const useContractNotification = () => {
  ...
  const [notification, setNotification] = useState<ContractNotification | null>(null);
  ...
  const checkNotification = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/contract/notification/${session.user.id}`);
...
export const useAdminContracts = () => {
  ...
  const adminSignContract = useCallback(async (studentId: string, contractId: string) => {
    try {
