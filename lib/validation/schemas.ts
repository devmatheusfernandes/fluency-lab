 ...
 */
function createSanitizedString(type: 'text' | 'html' | 'email' | 'url' | 'phone' | 'name' | 'id') {
 ...
 */
function sanitizeAfterValidation<T extends z.ZodString>(schema: T, type: 'text' | 'html' | 'email' | 'url' | 'phone' | 'name' | 'id') {
...
export const userValidationSchema = z.object({
  name: sanitizeAfterValidation(sanitizedSchemas.name.min(2, 'Nome deve ter pelo menos 2 caracteres').max(100, 'Nome muito longo'), 'name'),
  email: sanitizeAfterValidation(sanitizedSchemas.email.email('Email inválido').max(255, 'Email muito longo'), 'email'),
  phone: sanitizeAfterValidation(sanitizedSchemas.phone, 'phone').optional(),
  role: z.enum(['student', 'teacher', 'admin', 'manager', 'occasional_student']),
  profilePicture: sanitizeAfterValidation(sanitizedSchemas.url, 'url').optional(),
});
...
export const classValidationSchema = z.object({
  title: sanitizeAfterValidation(sanitizedSchemas.text.min(3, 'Título deve ter pelo menos 3 caracteres').max(200, 'Título muito longo'), 'text'),
  description: sanitizeAfterValidation(sanitizedSchemas.html.max(2000, 'Descrição muito longa'), 'html').optional(),
  teacherId: sanitizeAfterValidation(sanitizedSchemas.id.min(1, 'ID do professor é obrigatório'), 'id'),
  studentId: sanitizeAfterValidation(sanitizedSchemas.id.min(1, 'ID do estudante é obrigatório'), 'id'),
  startTime: z.date(),
  endTime: z.date(),
  ...
  notes: sanitizeAfterValidation(sanitizedSchemas.html.max(5000, 'Notas muito longas'), 'html').optional(),
});
...
export const announcementValidationSchema = z.object({
  title: sanitizeAfterValidation(sanitizedSchemas.text.min(5, 'Título deve ter pelo menos 5 caracteres').max(200, 'Título muito longo'), 'text'),
  content: sanitizeAfterValidation(sanitizedSchemas.html.min(10, 'Conteúdo deve ter pelo menos 10 caracteres').max(10000, 'Conteúdo muito longo'), 'html'),
  type: z.enum(['info', 'warning', 'success', 'error']),
  targetRoles: z.array(z.enum(['student', 'teacher', 'admin', 'manager', 'occasional_student'])).min(1, 'Pelo menos um role deve ser selecionado'),
...
export const settingsValidationSchema = z.object({
  ...
  timezone: sanitizeAfterValidation(sanitizedSchemas.text.max(50, 'Timezone inválido'), 'text'),
...
export const paymentValidationSchema = z.object({
  amount: z.number().positive('Valor deve ser positivo').max(10000, 'Valor muito alto'),
  currency: z.enum(['BRL', 'USD', 'EUR']).default('BRL'),
  method: z.enum(['credit_card', 'debit_card', 'pix', 'bank_transfer']),
  description: sanitizeAfterValidation(sanitizedSchemas.text.max(500, 'Descrição muito longa'), 'text').optional(),
...
export const feedbackValidationSchema = z.object({
  classId: sanitizeAfterValidation(sanitizedSchemas.id.min(1, 'ID da aula é obrigatório'), 'id'),
  rating: z.number().min(1, 'Avaliação mínima é 1').max(5, 'Avaliação máxima é 5'),
  comment: sanitizeAfterValidation(sanitizedSchemas.html.max(2000, 'Comentário muito longo'), 'html').optional(),
...
export const availabilityValidationSchema = z.object({
  teacherId: sanitizeAfterValidation(sanitizedSchemas.id.min(1, 'ID do professor é obrigatório'), 'id'),
  dayOfWeek: z.number().min(0, 'Dia da semana inválido').max(6, 'Dia da semana inválido'),
...
export const fileUploadValidationSchema = z.object({
  fileName: sanitizeAfterValidation(sanitizedSchemas.text.min(1, 'Nome do arquivo é obrigatório').max(255, 'Nome do arquivo muito longo'), 'text'),
  fileSize: z.number().positive('Tamanho do arquivo deve ser positivo').max(50 * 1024 * 1024, 'Arquivo muito grande (máximo 50MB)'),
  mimeType: z.string().regex(/^[a-zA-Z0-9][a-zA-Z0-9!#$&\-\^_]*\/[a-zA-Z0-9][a-zA-Z0-9!#$&\-\^_.]*$/, 'Tipo MIME inválido'),
...
export const searchValidationSchema = z.object({
  query: sanitizeAfterValidation(sanitizedSchemas.text.max(200, 'Consulta muito longa'), 'text').optional(),
  ...
  sortBy: sanitizeAfterValidation(sanitizedSchemas.text.max(50, 'Campo de ordenação inválido'), 'text').optional(),
...
export const authValidationSchema = z.object({
  email: sanitizeAfterValidation(sanitizedSchemas.email.email('Email inválido'), 'email'),
  password: z.string().min(8, 'Senha deve ter pelo menos 8 caracteres').max(128, 'Senha muito longa'),
  rememberMe: z.boolean().default(false),
  twoFactorCode: z.string().regex(/^[0-9]{6}$/, 'Código 2FA deve ter 6 dígitos').optional(),
});
...
export const passwordResetValidationSchema = z.object({
  token: sanitizeAfterValidation(sanitizedSchemas.text.min(1, 'Token é obrigatório'), 'text'),
