...
import { ClassTemplate } from '@/types/classes/class'; // Supondo que você exporte um tipo 'ClassTemplate'
...
export class ClassTemplateRepository {
    ...
    async upsert(studentId: string, templateData: ClassTemplate): Promise<void> {
        const docRef = templatesCollection.doc(studentId);
        await docRef.set(templateData, { merge: true }); // 'merge: true' atualiza campos sem sobrescrever o doc inteiro
...
// Exporta uma instância singleton para ser usada nos serviços
