...
export class VacationRepository {
 /**
   ...
   */
  async findByTeacherAndYear(teacherId: string, year: number): Promise<Vacation[]> {
  ...
  async findAllByTeacherId(teacherId: string): Promise<Vacation[]> {
    const snapshot = await vacationsCollection
      ...
      .get();
    console.log(`[REPOSITÓRIO vacations] A consulta para o professor ${teacherId} encontrou ${snapshot.docs.length} documentos.`);
