...
import { achievementDefinitions } from "@/services/achievementDefinitions";
...
export class AchievementRepository {
  ...
  async getStudentAchievements(studentId: string): Promise<StudentAchievement[]> {
    try {
      ...
      if (docSnap.exists) {
        ...
        const initialAchievements: StudentAchievement[] = achievementDefinitions.map(def => ({
  ...
  async updateStudentAchievements(
    ...
    try {
      ...
      const sanitizedAchievements = achievements.map((achievement) => {
        ...
        if (achievement.unlockedAt !== undefined && achievement.unlockedAt !== null) {
  ...
  async addStudentAchievement(
    ...
    try {
      ...
      await docRef.update({
        achievementsList: admin.firestore.FieldValue.arrayUnion(sanitizedAchievement)
      });
    } catch (error) {
