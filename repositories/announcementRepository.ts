...
export class AnnouncementRepository {
  ...
  async create(announcement: Omit<Announcement, 'id' | 'createdAt' | 'readBy'>): Promise<Announcement> {
    try {
      ...
      const cleanedAnnouncement = JSON.parse(JSON.stringify(newAnnouncement, (key, value) => {
        if (value === undefined) {
      ...
      await announcementsCollection.doc(cleanedAnnouncement.id).set(cleanedAnnouncement);
  ...
  async findById(id: string): Promise<Announcement | null> {
    try {
      ...
      console.error(`Error fetching announcement by ID ${id} in repository:`, error);
      throw error;
    }
  }
  ...
  async markAsRead(announcementId: string, userId: string): Promise<void> {
    try {
      ...
      console.error(`Error marking announcement ${announcementId} as read for user ${userId} in repository:`, error);
      throw error;
    }
  }

  async getAnnouncementsForUser(userId: string, userRole: UserRoles): Promise<Announcement[]> {
    try {
      ...
      console.error(`Error fetching announcements for user ${userId} with role ${userRole} in repository:`, error);
      // Provide guidance for the index error
      if (error.code === 9) { // FAILED_PRECONDITION
        console.error('Firestore index required. Create the composite index using the link in the error message.');
      }
      throw error;
    }
  }

  async getUnreadCountForUser(userId: string, userRole: UserRoles): Promise<number> {
    try {
      const announcements = await this.getAnnouncementsForUser(userId, userRole);
      return announcements.filter(a => !a.readBy.includes(userId)).length;
    } catch (error) {
      console.error(`Error counting unread announcements for user ${userId} with role ${userRole} in repository:`, error);
      throw error;
    }
  }
}