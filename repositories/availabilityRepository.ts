...
import { AvailabilityException, AvailabilitySlot } from "@/types/time/availability"; // Ajuste o caminho se necessário
...
export class AvailabilityRepository {
  ...
  async create(slotData: Omit<AvailabilitySlot, 'id'>): Promise<AvailabilitySlot> {
  ...
  async findByTeacherId(teacherId: string): Promise<AvailabilitySlot[]> {
    const snapshot = await this.availabilitiesRef.where("teacherId", "==", teacherId).get();
  ...
  async createException(slotId: string, teacherId: string, exceptionDate: Date): Promise<void> {
  ...
  async findExceptionsByTeacherId(teacherId: string): Promise<AvailabilityException[]> {
  ...
  async findAndDeleteException(originalSlotId: string, exceptionDate: Date): Promise<void> {
