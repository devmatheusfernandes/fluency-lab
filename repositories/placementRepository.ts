...
import { collection, query, orderBy, onSnapshot, getDocs } from 'firebase/firestore'; // Client-side imports
...
export class PlacementRepository {
  ...
  async getPlacementTestsByUserId(userId: string): Promise<PlacementTest[]> {
    try {
      ...
      const placementRef = adminDb.collection('users').doc(userId).collection('Placement');
   ...
   */
  subscribeToPlacementTests(userId: string, callback: (tests: PlacementTest[]) => void) {
    try {
