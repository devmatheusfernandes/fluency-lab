...
export class CreditService {
   ...
   */
  async grantCredits(request: GrantCreditRequest, grantedBy: string): Promise<RegularClassCredit> {
    ...
    // Only regular students can receive credits (not occasional students)
    if (student.role === 'occasional_student') {
      throw new Error('Estudantes ocasionais não podem receber créditos de aulas regulares.');
    }
  ...
  async getStudentCreditsBalance(studentId: string): Promise<RegularCreditsBalance> {
    ...
    // Categorize credits
    const availableCredits = credits.filter((c: RegularClassCredit) => !c.usedAt && new Date(c.expiresAt) > now);
    const expiredCredits = credits.filter((c: RegularClassCredit) => !c.usedAt && new Date(c.expiresAt) <= now);
    ...
    return {
      ...
      usedCredits: usedCredits.reduce((sum: number, c: RegularClassCredit) => sum + c.amount, 0),
  ...
  async useCredit(request: UseCreditRequest, usedBy: string): Promise<void> {
    ...
    const creditIndex = student.regularClassCredits.findIndex((c: RegularClassCredit) => c.id === creditId);
    if (creditIndex === -1) {
   ...
   */
  async findAvailableCredit(studentId: string, preferredType?: RegularCreditType): Promise<RegularClassCredit | null> {
    ...
    if (preferredType) {
      const creditOfType = balance.credits.find(c => c.type === preferredType && c.amount > 0);
      if (creditOfType) {
        return creditOfType;
      }
    }
    ...
    return balance.credits
      ...
      .sort((a: RegularClassCredit, b: RegularClassCredit) => new Date(a.grantedAt).getTime() - new Date(b.grantedAt).getTime())[0] || null;
  }
  ...
  async cleanupExpiredCredits(studentId: string): Promise<void> {
    ...
    const now = new Date();
    const activeCredits = student.regularClassCredits.filter((c: RegularClassCredit) => 
      c.usedAt || new Date(c.expiresAt) > now
    );
   ...
   */
  private async createCreditTransaction(transaction: CreditTransaction): Promise<void> {
    const transactionRef = adminDb.collection('creditTransactions').doc(transaction.id);
   ...
   */
  async getCreditTransactionHistory(studentId: string): Promise<CreditTransaction[]> {
